...
export class TeacherRepositoryMikroORM implements TeacherRepository {
  ...
  async getTeacherById(
    ...
    try {
      const result = await this.connection
        ...
        .findOne(new ObjectId(teacherId));
      return result;
    } catch (error) {
      console.log(error);
      return HttpStatus.INTERNAL_SERVER_ERROR;
  ...
  async createTeacher(
    ...
    try {
      const result = await this.connection.insertOne('teachers', {
        _id: null,
        ...payload,
      });
  ...
  async updateTeacher(
    ...
    try {
      await this.connection.deleteMany('teachers', {
