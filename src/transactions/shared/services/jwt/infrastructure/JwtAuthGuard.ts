...
const roleToDocumentationRole: { [key in Role]: DocumentationRoles } = {
  ...
  [Role.TEACHER_DEMO]: DocumentationRoles.TEACHER_DEMO_JWT,
...
export function GuardWithJwt(roles: Role[]) {
  const apiSecurities = roles.map((role) =>
